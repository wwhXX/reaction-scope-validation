...
def compute_cluster_centroid(cluster_points, metric='euclidean'):
    ...
    if metric == 'manhattan':
        ...
        # 对于Tanimoto距离，使用平均值作为centroid
        # 这是一个简化的方法，因为Tanimoto距离的真正centroid计算非常复杂
        ...
        if metric == 'tanimoto':
            ...
            for i in range(len(movable_centers)):
                ...
                if len(cluster_points) > 0:
                    ...
                    # Tanimoto相似度对c的梯度
                    # dS/dc = [p * denominator - p_dot_c * (2*c - p)] / denominator²
                    # 向量化计算
                    ...
                    for j, p in enumerate(cluster_points):
                        ...
                        # Tanimoto距离 D = 1 - S, 所以 dD/dc = -dS/dc
                        gradient_sum += -dS_dc
    ...
    def compute_repulsion_energy_gradients(centers, repulsion_points, strength, metric='euclidean'):
        ...
        for i, center in enumerate(centers):
            for repulsion_point in repulsion_points:
                ...
                if metric == 'tanimoto':
                    # 对于Tanimoto，使用距离的反比作为排斥函数
                    tanimoto_sim = tanimoto_similarity(center.reshape(1, -1), repulsion_point.reshape(1, -1))
                    tanimoto_dist = 1.0 - tanimoto_sim
                    tanimoto_dist = np.maximum(tanimoto_dist, 1e-10)  # 避免除零
                    
                    # 排斥能量: strength / distance
                    energy += strength / tanimoto_dist
                    
                    # 计算梯度: d/dc [strength / D] = d/dc [strength / (1-S)]
                    # = -strength * d/dc[-S] / (1-S)² = strength * (dS/dc) / (1-S)²
                    p = repulsion_point
                    c = center
