...
# Read sampling files
final_sampling = pd.read_csv('final_sampling_itr_5.csv')
final_drop = pd.read_csv('final_drop_itr_5.csv')
...
if sampling_indices:
    ...
    if blue_x:
        plt.scatter(blue_x, blue_y, c='#79b6da', s=125, alpha=0.8, edgecolors='white', linewidth=1, label='Final sampling (1700)')
    if orange_x:
        plt.scatter(orange_x, orange_y, c='#ff8040', s=125, alpha=0.8, edgecolors='white', linewidth=1, label='Final sampling (ZINC)')

# Add scatter plots for final_drop (gray)
if drop_indices:
    drop_x = [X_umap[i][0] for i in drop_indices]
    drop_y = [X_umap[i][1] for i in drop_indices]
    plt.scatter(drop_x, drop_y, c='gray', s=125, alpha=0.8, edgecolors='darkgray', linewidth=1, label='Final drop')
