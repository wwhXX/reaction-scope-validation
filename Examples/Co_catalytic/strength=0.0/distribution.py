...
if sampling_indices:
    ...
    if blue_x:
        plt.scatter(blue_x, blue_y, c='#79b6da', s=300, alpha=0.8, edgecolors='white', linewidth=1, label='Final sampling (Exp)')
    if orange_x:
        plt.scatter(orange_x, orange_y, c='#ff8040', s=300, alpha=0.8, edgecolors='white', linewidth=1, label='Final sampling (ZINC)')

# Add scatter plots for final_drop (gray)
if drop_indices:
    drop_x = [X_umap[i][0] for i in drop_indices]
    drop_y = [X_umap[i][1] for i in drop_indices]
    plt.scatter(drop_x, drop_y, c='gray', s=300, alpha=0.8, edgecolors='darkgray', linewidth=1, label='Final drop')
